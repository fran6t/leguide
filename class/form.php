...
class form extends My_sql{
	...
	function f_header_guide_logo($url_merchand,$url_img){
		...
		<table>
		<tr>
			<td align=\"left\"><br /><label>A propos</label></td>
			<td></td>
			<td></td>
		</tr>
		<tr>
			...
			<a href='http://www.prestashop.com/forums/viewthread/22501/modules_tiers/module_prestashop__leguide_dot_com__export_csv_pour_leguide_et_autres_comparateurs'>Export csv pour LeGuide et autres comparateurs</a></td>
			<td></td>
		</tr>";
	}
	...
	function f_header_credits_doc($url_site_base_prestashop, $cronKey, &$knownCatalogues) {
		...
		<tr>
			...
			Fabien Lahaulle,
			DSI 94,
			<a href='http://www.ktechnologie.com' title='Contactez-nous pour toute réalisation Prestashop'>KTechnologie</a>
			</td>
		</tr>
		";
		...
		$url_auto = 'http://'.$url_site_base_prestashop.'modules/leguide/cron.php?cronKey='.$cronKey;
		echo"
		<tr>
		...
		echo"
			<br>Ces URL doivent être rajoutées dans le module Crontab de Prestashop qui peut être téléchargé ici :<br>
			<a href='http://addons.prestashop.com/fr/outils-administration/671-crontab-for-prestashop.html' target='_blank'>http://addons.prestashop.com/fr/outils-administration/671-crontab-for-prestashop.html</a>
			<br><br>
		...
		";
		//	Remarques : le mode 'auto' désactive l'affichage de l'arborescence pour raccourcir le temps d'exécution du script<br><br>
	}
   ...
   function f_form_devise_shop() {
		...
		echo "<br>
			<tr>
				<td align=\"left\"><br /><label>Export</label></td>
				<td></td>
				<td></td>
			</tr>
			<tr>
		...
		try
		{
			// Fran6t 06/05/2013 Chez moi un prestashop debuter en 1.1 et maintenant en 1.5.4.1 il n'ya pas pas de prefix 
			// dans la valeur CURRENCY_DEFAULT c'est PS_CURRENCY_DEFAULT
			//$Resulats = parent::$this->mysql->TabResSQL('SELECT value FROM '.$this->ps_.'configuration where name = "'.$this->ps_.'CURRENCY_DEFAULT"');
			$Resulats = parent::$this->mysql->TabResSQL('SELECT value FROM '.$this->ps_.'configuration where name = "PS_CURRENCY_DEFAULT"');
			//echo 'SELECT value FROM '.$this->ps_.'configuration where name = "'.$this->ps_.'CURRENCY_DEFAULT"';
			...
			echo $e -> RetourneErreur('Impossible de sélectionner l\id de la devise par dfaut');
		}

		// Slectionne le code ISO et le cours
		try
		{
			$Resulats = parent::$this->mysql->TabResSQL("SELECT iso_code,conversion_rate FROM ".$this->ps_."currency where id_currency = '".$id_currency."'");
			foreach ($Resulats as $Valeur)
			{
	...
	// Langues utilises sur le shop
	function f_list_langue() {

		echo "<tr><td align=\"right\" ><br>Langue d'export</td>
		<td style=\"padding-left:15px;\"><br>
		<select name=\"lang\">";

		try
		{
			$Resulats = parent::$this->mysql->TabResSQL('SELECT id_lang,name,iso_code FROM '.$this->ps_.'lang WHERE active=1');
			foreach ($Resulats as $Valeur)
			{
				...
				$register_lang = parent::$this->tools_guide->f_get_value('lang_export','leguide');
	...
	function f_display_carrier() {
		...
		try
		{
			$Resulats = parent::$this->mysql->TabResSQL('SELECT id_carrier, name FROM '.$this->ps_.'carrier WHERE active=1 and deleted=0 GROUP BY name');
			foreach ($Resulats as $Valeur)
			{
				...
				if ($name == '0') {
					...
					$Resulats = parent::$this->mysql->TabResSQL($req);
					$name = $Resulats[0]['value'];
				}

				$register_carrier = parent::$this->tools_guide->f_get_value('livreur','leguide');
	...
	// Formulaire disponibilité en stock LEGUIDE
	//Avi - 12/07/2009 - Correction bug sauvegarde
	function f_disponibilite_leguide() {

		$disponibilite = parent::$this->tools_guide->f_get_value('disponibilite','leguide');
		...
		echo "
		<tr>
			<td align=\"right\">Phrase insérée renseignant la disponibilit&eacute;</td>
			<td style=\"padding-left:15px;\">
	...
	function f_zone() {
	   ...
	   echo "<tr>
	       ...
	       <select name=\"zone\">";
			...
			// Affichage des diffrentes zones
			try
			{
				$Resulats = parent::$this->mysql->TabResSQL("SELECT id_zone,name from ".$this->ps_."zone where active = 1");
				foreach ($Resulats as $Valeur)
				{
					$id_zone 	= $Valeur['id_zone'];
					$name_zone 	= $Valeur['name'];

					$frais = parent::$this->tools_guide->f_get_value('frais','leguide');
   ...
   // Délai de livraison leguide
   //Avi - 12/07/2009 - Correction bug sauvegarde
   function f_delai_livraison_leguide() {

   		$delai_livraison = parent::$this->tools_guide->f_get_value('delai_livraison','leguide');
	   ...
	   echo
	   "<tr>
	       <td align=\"right\">D&eacute;lai de livraison</td>
	       <td style=\"padding-left:15px;\">
   ...
   // Garantie leguide
   //Avi - 12/07/2009 - Correction bug sauvegarde
   function f_garantie_leguide() {

   		$garantie = parent::$this->tools_guide->f_get_value('garantie','leguide');
   ...
   // Etat leguide
   function f_etat_leguide() {

   		$etat = parent::$this->tools_guide->f_get_value('etat','leguide');
   ...
   function f_form_header($method,$name){
	...
	function f_url_rewriting() {
		...
		$url_rewriting = parent::$this->tools_guide->f_get_value('url_rewriting','leguide');
		if($url_rewriting=="on")
			{$checked = "checked";}
		else
			{$checked = "";}
	...
	// Avi - export par declinaison
	function f_makedeclinaison() {

		$makedeclinaison = parent::$this->tools_guide->f_get_value('makedeclinaison','leguide');
		if($makedeclinaison=="on")
			{$checked = "checked";}
		else
			{$checked = "";}
	  	echo "
		<tr>
			<td align=\"left\"><br /><br /><label>Articles</label></td>
			<td></td>
			<td></td>
		</tr>
		<tr>
	...
	function f_usefreeshipping() {
		...
		$usefreeshipping = parent::$this->tools_guide->f_get_value('usefreeshipping','leguide');
		if($usefreeshipping=="on")
			{$checked = "checked";}
		else
			{$checked = "";}
	  	echo "
		<tr>
			<td align=\"right\">Utiliser la gratuité des frais de port (&agrave; tester)</td>
	...
	function f_exportallproduct() {
		...
		$exportallproduct = parent::$this->tools_guide->f_get_value('exportallproduct','leguide');
		if($exportallproduct=="on")
			{$checked = "checked";}
		else
			{$checked = "";}
	  	echo "
	...
	//Avi - 12/07/2009 - Rajout choix type de description
	function f_description_courte() {
		$description_courte = parent::$this->tools_guide->f_get_value('description_courte','leguide');
		if($description_courte=="on")
			{$checked = "checked";}
		else
			{$checked = "";}
	  	echo "
	...
	//Avi - 12/07/2009 - Rajout choix export des produits inactifs egalement
		function f_actif_only() {
			$actif_only = parent::$this->tools_guide->f_get_value('actif_only','leguide');
		if($actif_only=="on")
			{$checked = "checked";}
		else
			{$checked = "";}
   ...
   function f_CaracteristiquesFichier() {
		echo "
		<tr>
			<td><br /><br /><label>Caract&eacute;ristiques Fichier</label></td>
			<td></td>
			<td></td>
		</tr>";
   }
   ...
   function f_nom_fichier() {
   		$nom_fichier = parent::$this->tools_guide->f_get_value('nom_fichier','leguide');
   ...
   function f_destDir() {
   		$destDir = parent::$this->tools_guide->f_get_value('destDir','leguide');
   ...
   // Separateur dans le fichier csv	-	Obsolète
   function f_separateur() {

   		$separateur = parent::$this->tools_guide->f_get_value('separateur','leguide');
   ...
   // Extension du fichier gnr
   function f_extension_fichier() {
   		$extension_fichier = parent::$this->tools_guide->f_get_value('extension_fichier','leguide');
   ...
   function f_DisplayComparateurs() {
   		$nom_comparateur = parent::$this->tools_guide->f_get_value('nom_comparateur','leguide');
		if($nom_comparateur<>'')
   ...
   function f_Informations($module_name, $url_site_base_prestashop){
		echo "
			<tr>
				<td><br /><br /><label>Informations sur les comparateurs</label></td>
				<td></td>
				<td></td>
			</tr>
			<tr>
			...
			<tr>
		   	...
		   	<tr>
				<td align=\"right\">Voir aussi module Google Shopping</td>
				<td colspan=\"2\" style=\"padding-left:15px;\" class=\"links\"><a href='http://www.prestashop.com/forums/viewthread/76681/P330/#444032' TARGET=\"_blank\">http://www.prestashop.com/forums/viewthread/76681/P330/#444032</a></td>
			</tr>
		   	<tr>
		       <td></td>
		       <td></td>
	...
	//30/05/2011 KTecknologie.com: : Génération par tranches pour serveurs mutualisés
   function f_displayCatalog() {
		$displayCatalog = parent::$this->tools_guide->f_get_value('displayCatalog','leguide');
		(($displayCatalog) ? $checked = 'checked' : $checked = '');
	  	echo "
		<tr>
			<td align=\"right\"><br /><br /><label>Affichage</label></td>
			<td></td>
			<td></td>
		</tr>
		<tr>
   ...
   function f_onMutu() {
		...
		$onMutu = parent::$this->tools_guide->f_get_value('onMutu','leguide');
		($onMutu=="on") ? $onMutuChecked = 'checked' : $onMutuChecked = '';

		$onMutuStep = parent::$this->tools_guide->f_get_value('onMutuStep','leguide');

		$onMutuOnceaDay = parent::$this->tools_guide->f_get_value('onMutuOnceaDay','leguide');
		($onMutuOnceaDay=="on") ? $onMutuOnceaDayChecked = 'checked' : $onMutuOnceaDayChecked = '';

	  	echo "
		<tr>
			<td nowrap=\"nowrap\" align=\"right\"><br /><br /><label>Serveurs mutualisés</label></td>
			<td></td>
			<td></td>
		</tr>
		<tr>
	...
	function f_displayGroups() {
		...
		$id_group = parent::$this->tools_guide->f_get_value('id_group','leguide');
		echo '<tr>
