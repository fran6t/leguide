<?php
	...
	// /////////////////////////////////////////
	// Fran6t 06/05/2013 ajout du parametre id_lang
	function base_arbre($ps_,$module_name,$id_lang){
		...
		// Fran6t 06/05/2013 ajout mise en variable id_lang
		$sql = 'SELECT * from '.$ps_.'category LEFT JOIN '.$ps_.'category_lang ON '.$ps_.'category.id_category = '.$ps_.'category_lang.id_category WHERE level_depth=0 and id_parent=0 and id_lang='.$id_lang.' and active=1';
		echo $sql; 
		//error_log('--'.$sql);
		//$trace = "$sql\r\n---\r\n";
		$res = mysql_query($sql) or die('Erreur SQL !<br>'.$sql.'<br>'.mysql_error());
	...
	function branche($id_category, $level, $ps_, $module_name, $id_lang){
		...
		// Fran6t 06/05/2013 ajout mise en vairiable id_lang
	...
	function f_calc_fdp($usefreeshipping,$shipping_method,$delivery_price,$seuil_prix_fdp_offert,$seuil_poids_fdp_offert,$prix,$poids){
		...
		// Fran6t 07/05/2013 si dans l'admin module on coche utiliser la gratuité des frais de port
		// Quelque chose de bizarre chez moi depuis l'admin module usfreeshipping vaut on et lors de l'appel via cron c'est 1
		//echo "usefreeshipping=".$usefreeshipping;
		if ($usefreeshipping == 1 || $usefreeshipping == "on" ){
			return 0;
		}
